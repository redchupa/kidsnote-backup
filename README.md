...
>
> | 알림장 수 | 예상 시간 | 필요한 run 횟수 |
> |---|---|---|
> | 50개 미만 (몇 달치) | 5~30분 | 1회 |
> | 100~200개 (반년) | 4~10시간 | 1~2회 |
> | 300~400개 (1년치) + 사진 많음 | **18~30시간** | **3~5회** |
>
> GitHub-hosted runner는 단일 job **6시간 하드 캡**이 있어서 한 번에 못 끝낼 수 있어요. 컴퓨터를 꺼도 GitHub Actions가 클라우드에서 계속 돌아갑니다.

**왜 6시간 캡에 걸리면 자동 재개 안 되나요**:
- GitHub Actions는 cap 초과 시 강제 종료
- 다음 실행은 사용자가 수동으로 트리거해야 함 (Actions 탭 → `Run workflow` 한 번 더 클릭)
- **걱정 마세요**: dedup이 이미 백업된 알림장을 자동 skip하므로 다음 실행은 **이어서** 진행됩니다 (몇 초~몇 분 만에 멈춘 지점부터 재개)

**처음 백업의 두 단계**:
1. **알림장·공지·앨범·식단 publish** — 새 페이지 생성 (시간 大, 사진 업로드 포함)
2. **7개 대시보드 자동 생성** — 통계/추억/영양 + 4개 LLM 페이지 (성장 스토리/마일스톤/관심사/감사 카드). 1년치 LLM 분석은 **추가로 1~2시간** 소요

> 💡 **`force_refresh` 옵션** (고급): 코드 업데이트 후 기존 알림장의 callout(자녀 일기/부모 편지)도 새 LLM prompt로 다시 만들고 싶으면, `Run workflow`에서 `force_refresh`를 `true`로 설정. **첫 백업과 동일한 시간 소요** (모든 페이지를 archive 후 재발행).
>
> 일반 사용자는 `force_refresh=false` (기본값)로 충분합니다. 매일 1~2회 실행 시 1~5분이면 끝납니다.

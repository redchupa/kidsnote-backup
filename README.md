...
> ⚠️ **첫 백업은 시간이 걸립니다**:
> - 알림장 50개 미만: **5~15분**
> - 1년치 (300~400개) + 사진 많음: **6~18시간** (GitHub-hosted runner는 단일 job 6h 하드 캡이 있어서 한 번에 끝나지 않을 수 있음. 끊겨도 dedup이 작동해서 다음 실행 때 이어서 처리)
>
> 컴퓨터는 꺼도 됩니다. GitHub Actions가 클라우드에서 계속 돌아갑니다.

**처음 백업의 두 단계**:
1. 알림장·공지·앨범·식단 publish — 새 페이지 생성 (시간 大)
2. 7개 대시보드 자동 생성 — 통계/추억/영양 + 4개 LLM 페이지 (성장 스토리/마일스톤/관심사/감사 카드)

만약 6시간 cap에 걸려서 알림장이 다 못 publish됐다면, 그냥 같은 워크플로를 한 번 더 실행하면 dedup이 이미 백업된 페이지를 skip하고 나머지만 처리합니다. 보통 1~3 차례면 완료됩니다.

> 💡 **`force_refresh` 옵션** (고급): 코드 업데이트 후 기존 알림장의 callout(자녀 일기/부모 편지)도 새 LLM prompt로 다시 만들고 싶으면, `Run workflow`에서 `force_refresh`를 `true`로 설정. 모든 페이지를 archive 후 재발행하므로 첫 백업과 동일한 시간 소요.

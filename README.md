...
1년치(보통 300개 안팎)가 노션 DB 한 곳에 자동 정렬됩니다.
...
5분 정도 기다린 후 노션 DB로 가서 알림장 3개가 들어왔는지 확인:
- 페이지 본문 OK
- 사진 OK
- 동영상/파일 첨부물 있으면 OK
...
> ⚠️ **주의**: 1년치(300+개) + 사진 많은 알림장이면 **1~3시간** 걸릴 수 있습니다. 컴퓨터를 꺼도 GitHub Actions가 계속 돌아갑니다.
...
- 페이지 클릭하면 본문 + 사진 + (있으면) 동영상 + 첨부파일 정상 표시
- 사진 클릭하면 원본 화질로 열림
...
가족·친척과 백업한 알림장을 함께 보고 싶다면, 노션의 "웹에 게시" 기능으로 공개 URL을 만들 수 있습니다. **로그인 없이** 누구나 링크로 접근 가능합니다.

> ⚠️ **신중하게 결정하세요**: 자녀 이름, 어린이집 정보, 사진(얼굴 포함)이 인터넷에 공개됩니다. 가족 그룹에만 공유할 거면 노션 워크스페이스 게스트 초대 방식을 권장 (아래 "비공개 공유"). **공개 URL은 검색엔진에 노출될 수 있습니다.**

## 옵션 A — 가족·친척에게만 공유 (비공개, 권장)

가장 안전한 방법. 노션 게스트로 초대해서 로그인한 사람만 보기 가능.

1. DB 페이지 우측 상단 **`공유`** (영문: `Share`) 클릭
2. **`사용자, 그룹, 통합 추가`** 입력란에 가족의 이메일 입력
3. 권한 **`보기 가능`** 선택 (편집 막힘)
4. **`초대`** 클릭

가족의 이메일로 초대장이 가고, 노션 계정만 있으면 로그인해서 접근 가능합니다. **검색엔진 노출 없음**, **링크 도용 불가**.

## 옵션 B — 완전 공개 웹 페이지 (누구나 URL로 접근)

블로그처럼 누구든 링크로 볼 수 있게 하려면:

1. DB 페이지 우측 상단 **`공유`** (영문: `Share`) 클릭
2. 팝업 우측 상단 **`웹에 게시`** (영문: `Publish`) 탭 클릭
3. **`웹에 게시`** 토글 ON
4. 게시된 공개 URL이 표시됨 (예: `https://yourname.notion.site/...`) — **복사**
5. 옵션 추가 설정 (필요 시):
   - **`링크 만료`**: 일정 기간 후 자동 비공개 (가족 행사 후 공개 종료 등)
   - **`편집 허용`**: ❌ 절대 ON 금지 (다른 사람이 페이지 수정 가능해짐)
   - **`복제 허용`**: 다른 사람이 본인 노션에 복사 가능. 자녀 사진 복제되는 걸 막으려면 OFF
   - **`댓글 허용`**: 로그인한 노션 사용자가 댓글 작성 가능
   - **`검색엔진 색인`**: 구글 검색에 노출. 자녀 정보면 OFF 강력 권장

6. 공유할 사람에게 URL 전달

### 공개 게시 후 비공개로 되돌리기

같은 메뉴에서 **`웹에 게시`** 토글 OFF → 즉시 비공개. URL 무효화.

### ⚠️ 공개 게시 전 체크리스트

- [ ] 자녀 사진을 인터넷에 공개해도 괜찮은가?
- [ ] 어린이집 이름·반 이름이 본문에 적혀 있는데 공개해도 괜찮은가?
- [ ] 검색엔진(`구글 검색에 키즈노트 #1234`)에서 발견되어도 괜찮은가? → 발견되기 싫으면 `검색엔진 색인` OFF
- [ ] URL을 받은 사람이 그 URL을 다른 사람에게 전달해도 괜찮은가?

가족·친척 정도만 본다면 **옵션 A(게스트 초대)가 훨씬 안전**합니다.
...
4. 5분 정도 기다림 (새 알림장만 처리, dedup이 나머지 자동 skip)
5. 끝!
...
**아쉽게도 안 됩니다.** 키즈노트의 로그인 페이지가 SPA(JavaScript 기반)로 바뀐 후, 자동 로그인 방법이 없어졌습니다. sessionid 쿠키를 수동으로 갱신해야 해서 **수동 실행만 가능**합니다.

다행히 한 번 셋업하면:
- 매주 한 번씩 워크플로 실행 (한 번 클릭, 30초)
- 한 달에 한 번 쿠키 갱신 (3분)
...
- GitHub Actions (ubuntu-latest, 6h timeout)
...
- `notion_mirror.py` — 노션 DB 쓰기 + 이미지 압축 + EXIF strip + 비디오/파일 업로드
